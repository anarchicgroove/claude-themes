...
(function () {
   ...
   *
   *  CHANGELOG v1.3.6
   *    · Share, take three — and this one's a real bug, not a guess.
   *      The hunt began at document.querySelector('header'): the FIRST
   *      header in the document, singular. Every CSS rule that works
   *      keys off `header` as a selector, which matches EVERY header on
   *      the page. So if claude.ai renders more than one, the styling
   *      lands on the right bar while the hunt searches the wrong one.
   *      Symptom matches exactly: chevron fixed, Share never tagged.
   *      Now every header is searched, then the whole document as a
   *      fallback in case Share isn't in a header at all.
   *    · The walk-up now stops at the first ancestor that actually
   *      PAINTS rather than the first button/link. If the visible pill
   *      is a wrapper div and the button inside it is transparent,
   *      v1.3.5 would have tagged the invisible one.
   *
   *  CHANGELOG v1.3.5
   *    · Share, take two. v1.3.4's hunt only looked at <button> and <a>
   *      inside the header and only accepted an exact "Share" — and the
   *      original v1.3.1 rule missing Share is itself evidence Share is
   *      neither of those tags. The hunt now widens to any element,
   *      falls back to aria-label, then walks UP to the control that
   *      actually paints — capped at half the bar's width, because that
   *      uncapped walk is exactly what bloated v1.3.2.
   *    · Share is conditional UI: it doesn't exist until Claude's first
   *      message lands. The observer already covers that, but the hunt
   *      is now throttled to once per 400ms so it can't burn battery
   *      re-scanning on every token of a streaming reply.
   *    · CSS keys off .sc-share alone (doubled for specificity) rather
   *      than `header button.sc-share`, so it still paints if Share
   *      turns out to live outside <header>.
   *
   *  CHANGELOG v1.3.4
   *    · The chevron beside the chat title loses its pale pill. The
   *      culprit rule was `header button[class*="border"]`, written for
   *      Share back in v1.3.1 — Share never carried a border class, the
   *      chevron did, so the rule painted the one thing it wasn't for.
   *      Replaced with "icon-only header controls ride bare".
   *    · Share finally gets its peach gradient. It has no stable class
   *      or aria-label either, so it's found by its word and tagged
   *      .sc-share in JS (THE SHARER), the same trick the docker uses
   *      for the disclaimer. Gradient is one step lighter than the
   *      title pill so the two read as a set, title still leading.
   *
   *  CHANGELOG v1.3.3
   *    · Revert of v1.3.2. This file is v1.3.1's styling exactly; the
   *      number only goes up because userscript managers won't install
   *      a lower version over a higher one.
   *    · v1.3.2 tried to tag the whole title container as one pill so
   *      the chevron would ride on the gradient. The walk-up grabbed a
   *      container wider than the pill, so the gradient bloated and
   *      went pale. Approach abandoned.
   *
   *  (The OPEN item logged here in v1.3.3 — the chevron's stray pill —
   *  is closed by v1.3.4 above.)
   *
   *  CHANGELOG v1.3.1
   *    · The chat-title pill in the top bar goes warm peach gradient
   *      instead of near-white. Share gets a much fainter wash of the
   *      same peach so the two read as a set without competing.
   *
   *  CHANGELOG v1.3.0
   *    · Bradley Hand in the user's message bubbles and the composer.
   *      Claude's own text is deliberately left as the site default.
   *
   *  CHANGELOG v1.2.0
   *    · FIX: expanded images vanished. v1.0.0 put position:relative on
   *      every direct child of <body> so the app would sit above the sky.
   *      claude.ai renders its image viewer into a portal div appended to
   *      the end of <body> — that rule caught the portal too and made it
   *      the positioning anchor, so the lightbox rendered at the bottom
   *      of the document instead of over the viewport. The rule is gone;
   *      the sky sits at z-index -1 now and touches nothing.
   *    · The "Show more" fade inside collapsed messages is now bubble
   *      pink instead of claude.ai's neutral grey (Section 7b).
   *
   *  CHANGELOG v1.1.0
   *    · Cloud lobes rebuilt as a border-image (see Section 7). Fixes
   *      BOTH the mirrored-edge asymmetry and the broken "Show more"
   *      collapse — the v1.0.0 lobes needed overflow:visible on the
   *      wrapper, which was the very thing clipping long messages.
   *    · Composer dock: transparent -> soft fade to blush, so scrolled
   *      text dissolves instead of colliding with the disclaimer.
   *    · Disclaimer tagged by a text-hunter and given a white halo.
   *
   *  PALETTE
