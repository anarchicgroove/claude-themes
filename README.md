claude-themes 🌸
Pretty CSS themes for claude.ai — soft palettes, custom fonts, and message bubbles that don't look like everyone else's.
...
Install → Stylus version · Userscripts version
---
How to install
<details>
<summary><b>💻 Stylus — Firefox or Chrome on desktop</b></summary>
Open your browser's add-on store and search for Stylus. Install it.
(Careful — there's an older extension called Stylish with a similar name. You want Stylus.)
Click the Stylus version link for the theme you want, above.
Stylus will open its own install page showing the theme's name and details. Click Install style.
Reload claude.ai. Done. 🎉
...
Install Firefox from the Play Store if you don't have it.
In Firefox, open the ⋮ menu → Add-ons → find and install Stylus.
Tap the Stylus version link for the theme you want, above.
Tap Install style when Stylus offers.
Reload claude.ai.
...
Install Userscripts from the App Store. It's free.
Open the Userscripts app and set a scripts directory — it'll walk you through picking or making a folder. Remember where you put it.
Open Safari → tap the ᴀA icon in the address bar → Manage Extensions → turn on Userscripts.
Tap ᴀA again → Userscripts → set claude.ai to Always Allow.
Then, to add a theme:
Tap the Userscripts version link for the theme you want, above.
On that GitHub page, tap the ⋯ menu → Raw file content → Download.
Open the Files app → Downloads → press and hold the downloaded file → Move → put it in your Userscripts folder.
Reload claude.ai. 🎉
...
Released under the MIT License — use them, change them, share them.
